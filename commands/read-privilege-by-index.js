...
module.exports = {
    register: (program) => {
        program
            .command('read-privilege-by-index <deviceSn> <controllerIp> <indexNumber>')
            .description('Read privilege with the specified index number')
            ...
            .action(async (deviceSn, controllerIp, indexNumber, options) => {
                ...
                console.log(`Reading privilege at index ${indexNumber} from controller SN: ${targetDeviceSn} at IP: ${targetControllerIp}`);
                const socket = dgram.createSocket('udp4');
                // Bind to a random available port, don't bind to the target port
                socket.bind();

                const readPrivilegePacket = Buffer.alloc(PACKET_LENGTH, 0);
                readPrivilegePacket.writeUInt8(TYPE_BYTE, 0);
                readPrivilegePacket.writeUInt8(0x5C, 1); // Function ID: Read the Privilege with the specified index number
                readPrivilegePacket.writeUInt32LE(parseInt(targetDeviceSn), 4); // Device serial number
                readPrivilegePacket.writeUInt32LE(parseInt(indexNumber), 8); // Privilege index number

                try {
                    const response = await sendUdpCommand(socket, readPrivilegePacket, targetControllerIp, options.port);
                    if (response && response.readUInt8(1) === 0x5C) {
                        const returnedCardNumber = response.readUInt32LE(8);
                        if (returnedCardNumber === 0 || returnedCardNumber === 0xFFFFFFFF) {
                            console.log(`No privilege found at index ${indexNumber}.`);
                        } else {
                        ...
                        console.error('Failed to read privilege by index or unexpected response.');
                    }
                } catch (error) {
                    console.error('Error during reading privilege by index:', error.message);
                } finally {
                    socket.close();
